// Combined script.js - Episodes 1..10 - Android clickable fixes + new stat model + effect scaling
...
let timer = 10;
let timerInterval;
let scenarios = []; // current episode scenarios
const totalEpisodes = 10;
const scenariosPerEpisode = 20; // show 20 scenarios per episode
...
// ======== Utility: effect scaling per your rules ========
function scaleEffectValue(val) {
  ...
  if (val === 4) return 5;
  if (val === -4) return -5;
  // 3 -> 3, 2 -> 2, -3 -> -3, -2 -> -2, 1/0/-1 remain as-is
  return val;
}

// Apply an effect object/number to player stats
function applyEffectToPlayerByKey(effect) {
  // effect may be number or object
  if (typeof effect === "number") {
    // apply to stat mapped by scenario.stat (handled elsewhere)
    return effect;
  }
  // object with keys like skill, mentor, fan, team
  const scaled = {};
  for (const k in effect) {
    scaled[k] = scaleEffectValue(effect[k]);
  }
  return scaled;
}

// ======== Episode scenario pools (Episodes 1-10)
// NOTE: options show text only; numeric effects are stored in effect fields and will be scaled when applied.
// Episode pools are based on the data you provided + episode10 objects where needed.
// ======== Episode 1 Scenario Pool ========
let episode1ScenarioPool = [
  {
